...
def draw():
    ...
    if(game_over):
        canvas.create_text(window_width/2, window_height/2, fill="white", font="Times 30 bold", text=f"Game Over: {score}")
        canvas.create_text(window_width/2, window_height/2 + 30, fill="white", font="Times 15", text=f"Press Space to Restart")
        return

    canvas.create_text(window_width/2, 10, fill="white", font="Times 15 bold", text=f"Score: {score}")
