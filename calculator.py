...
for row in range(row_count):
    for column in range(column_count):
        ...
        button = tkinter.Button(frame, text=value, font =("Ariel", 30), 
                                width=column_count-1, height=1, command=lambda  
                                value=value: button_clicked(value))

        if value in top_symbols:
            button.config(foreground=color_black, background= color_light_gray)
        elif value in right_symbols:
            button.config(foreground=color_white, background= clolor_orange)
        else:
...
def button_clicked(value):
    ...
    if value in right_symbols:
        ...
        if value == "+/-":
            temp = float(label["text"]) * -1
            label["text"] = remove_zero_decimal(temp)
        if value == "%":
            temp = float(label["text"]) / 100
            label["text"] = remove_zero_decimal(temp)
        elif value == "√":
            print("clicked")
            temp = math.sqrt(float(label["text"]))
            label["text"] = remove_zero_decimal(temp)
    else:
        if value ==".":
